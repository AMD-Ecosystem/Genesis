...
def _torch_get_gpu_idx(device):
    if sys.platform == "linux":
        ...
        nvidia_gpu_interface_path = "/proc/driver/nvidia/gpus/"
        # AMD path was already returned above via `if torch.version.hip: return device`.
        # Below handles NVIDIA. Use os.path.exists guard (amd-integration) plus
        # WSL2-friendly try/except (upstream).
        if os.path.exists(nvidia_gpu_interface_path):
            ...
            try:
                ...
                warnings.warn(
                    f"'{nvidia_gpu_interface_path}' is not available. Multi-GPU support will be disabled. "
