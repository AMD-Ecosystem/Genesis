...
# =====================================================================================================================
#
# Two implementations of the linesearch live below, selected at compile time by the static dispatcher
# `func_linesearch_batch` based on `static_rigid_sim_config.backend`:
#
#   * `_hbm` variants (CPU): verbatim restoration of the pre-`perf/monolith-hbm-reread-reduction` baseline
#     (genesis @ 2b519a8c). All hot constraint state is read from / written to global memory; no
#     `qd.simt.block.SharedArray` allocations. Required because the CPU JIT (LLVM ORC) does not currently
#     register implementations of `shared_array_t0_*` runtime symbols, so any SharedArray allocation
#     reachable from a CPU-compiled kernel link-fails with "Symbols not found: shared_array_t0_s*".
#
#   * `_opt` variants (GPU): the LDS-staged Tier 1b/2/4 optimization shipped in this branch — see
#     `PERF_MONOLITH_HBM_REREAD_REDUCTION.md`.
#
# Keep the two implementations behaviorally identical (same numerical sequence, same ls_it/ls_result side
# effects, same return signature for the batch function); the only differences are where the hot working
# set lives (HBM vs LDS) and whether `quad_gauss / eq_sum` are written to constraint_state vs threaded as
# scalar locals.


@qd.func
def func_ls_init_and_eval_p0_hbm(
    i_b,
    entities_info: array_class.EntitiesInfo,
    dofs_state: array_class.DofsState,
    constraint_state: array_class.ConstraintState,
    ...
    """CPU/HBM variant of `func_ls_init_and_eval_p0_opt`.

    Restores the pre-`perf/monolith-hbm-reread-reduction` body (genesis @ 2b519a8c). Reads `search`,
    `Jaref`, `jv`, `efc_D` from global memory; writes `quad_gauss[0..2]` and `eq_sum[0..2]` to
    `constraint_state` for the subsequent linesearch evaluators to re-read. Used only on the CPU
    backend, where `qd.simt.block.SharedArray` does not link in the JIT runtime."""
...
@qd.func
def func_ls_point_fn_hbm(
    i_b,
    alpha,
    constraint_state: array_class.ConstraintState,
    rigid_global_info: array_class.RigidGlobalInfo,
):
    """CPU/HBM variant of `func_ls_point_fn_opt` — reads Jaref/jv/efc_D and quad_gauss/eq_sum from global."""
    ne = constraint_state.n_constraints_equality[i_b]
    nef = ne + constraint_state.n_constraints_frictionloss[i_b]
    n_con = constraint_state.n_constraints[i_b]
...
@qd.func
def func_ls_point_fn_3alphas_hbm(
    i_b,
    alpha_0,
    alpha_1,
    alpha_2,
    ...
    """CPU/HBM variant of `func_ls_point_fn_3alphas_opt` — three-alpha batched eval, all reads from global."""
    ne = constraint_state.n_constraints_equality[i_b]
    nef = ne + constraint_state.n_constraints_frictionloss[i_b]
    n_con = constraint_state.n_constraints[i_b]
...
def func_linesearch_batch_hbm(
    ...
    """CPU/HBM variant of `func_linesearch_batch` — verbatim restoration of the pre-LDS-staging baseline
    (genesis @ 2b519a8c). Calls the `_hbm` inner functions; no `qd.simt.block.SharedArray` allocations,
    so it links cleanly under the CPU JIT (where `shared_array_t0_*` runtime symbols are not registered).

    Behaviorally identical to `func_linesearch_batch_lds`: same `gtol` derivation, same Phase 1/2/3
    control flow, same `ls_result` codes, same return semantics for `res_alpha`."""
    n_dofs = constraint_state.search.shape[0]
    ...
    if snorm < rigid_global_info.EPS[None]:
        ...
        p0_alpha, p0_cost, p0_deriv_0, p0_deriv_1 = func_ls_init_and_eval_p0_hbm(
            i_b,
            entities_info=entities_info,
            dofs_state=dofs_state,
            constraint_state=constraint_state,
        ...
        p1_alpha, p1_cost, p1_deriv_0, p1_deriv_1 = func_ls_point_fn_hbm(
            ...
            if not done:
                ...
                if not done:
                    ...
                    while constraint_state.ls_it[i_b] < rigid_global_info.ls_iterations[None]:
                        costs, grads, hess = func_ls_point_fn_3alphas_hbm(
...
def func_linesearch_batch_lds(
    ...
    """GPU/LDS variant of `func_linesearch_batch` — Tier 1b/2/4 of `perf/monolith-hbm-reread-reduction`.

    Allocates four `qd.simt.block.SharedArray` tiles (Jaref/jv/efc_D/search) and threads them through
    the inner linesearch evaluators, eliminating per-evaluation HBM rereads of the working set. Must
    only be reached on GPU backends; the CPU JIT does not link `qd.simt.block.SharedArray`."""
...
def func_linesearch_batch(
    ...
    """Backend-dispatched linesearch entry-point.

    `qd.simt.block.SharedArray` is not currently linkable in the CPU JIT runtime: any kernel that
    reaches a SharedArray allocation on the CPU backend fails at JIT-link time with
    "Symbols not found: shared_array_t0_s*". Dispatch at compile time on the static `backend` so the
    CPU-compiled binary never sees the LDS allocations or the LDS-using inner functions, while GPU
    backends keep the Tier 1b/2/4 LDS-staging optimization that gives the +7.21% benchmark gain."""
    res_alpha = gs.qd_float(0.0)
    if qd.static(static_rigid_sim_config.backend == gs.cpu):
        res_alpha = func_linesearch_batch_hbm(
            i_b,
            entities_info=entities_info,
            dofs_state=dofs_state,
            rigid_global_info=rigid_global_info,
        ...
        res_alpha = func_linesearch_batch_lds(
            i_b,
            entities_info=entities_info,
            dofs_state=dofs_state,
            rigid_global_info=rigid_global_info,
